...
void main_menu::init_strings()
{
    ...
    vMenuItems.clear();
    vMenuItems.push_back(pgettext("Main Menu", "<M|m>OTD")); // 0
    vMenuItems.push_back(pgettext("Main Menu", "<N|n>ew Game")); // 1
    vMenuItems.push_back(pgettext("Main Menu", "Multiplayer")); // 2
    vMenuItems.push_back(pgettext("Main Menu", "Lo<a|A>d")); // 3
    vMenuItems.push_back(pgettext("Main Menu", "<W|w>orld")); // 4
    vMenuItems.push_back(pgettext("Main Menu", "<S|s>pecial")); // 5
    vMenuItems.push_back(pgettext("Main Menu", "Se<t|T>tings")); // 6
    vMenuItems.push_back(pgettext("Main Menu", "H<e|E|?>lp")); // 7
    vMenuItems.push_back(pgettext("Main Menu", "<C|c>redits")); // 8
    vMenuItems.push_back(pgettext("Main Menu", "<Q|q>uit")); // 9
...
bool main_menu::opening_screen()
{
    ...
    while (!start) {
        ...
        if (layer == 1) {
            ...
            if ((sel1 == 0 || sel1 == 8) && (action == "UP" || action == "DOWN" ||
            ...
            if ((action == "UP" || action == "CONFIRM") && sel1 != 0 && sel1 != 8) {
                if (sel1 == 7) {
...
bool main_menu::multiplayer_tab() {
    return false;
}
...
bool main_menu::load_character_tab(bool transfer)
{
    ...
    while (!start && sel1 == 3 && (layer == 2 || layer == 3)) {
        print_menu(w_open, transfer ? 4 : 3, menu_offset);
        if (layer == 2 && sel1 == 3) {
...
{
    while (sel1 == 4 && (layer == 2 || layer == 3 || layer == 4)) {
        print_menu(w_open, 4, menu_offset);
